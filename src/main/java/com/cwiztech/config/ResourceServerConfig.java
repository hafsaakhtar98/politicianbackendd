		...
		resource.setConfigurers(
				Collections.<ResourceServerConfigurer>singletonList(new ResourceServerConfigurerAdapter() {
					...
					public void configure(HttpSecurity http) throws Exception {
						...
						http.requestMatchers().antMatchers("/activity", "/activity/**").and().authorizeRequests().anyRequest()
						.access("#oauth2.hasScope('write')");

						http.requestMatchers().antMatchers("/politician", "/politician/**").and().authorizeRequests().anyRequest()
						.access("#oauth2.hasScope('write')");
					}
				}));
